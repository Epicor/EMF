 ...
 * version:1.0.8-dev.121 built: 07-09-2016
*/
(function() {
    'use strict';
...
angular.module('ep.templates').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('src/components/ep.accordion.menu/ep-accordion-menu-item_template.html',
    "<div class=\"clearfix list-group-item\"><div class=\"clearfix ep-accordion-menu-animate\" id=mnu_{{item.id}} ng-class=\"{ 'ep-accordion-expanded': item.isExpanded && item.menuitems.length }\" ng-click=\"item.isExpanded = !item.isExpanded\"><div id=menuItem class=\"list-group-item row\" ng-if=\"!(item.menuitems && item.menuitems.length)\" ng-click=\"navigate(item, false, $event)\"><!-- Menu item caption/text --><span class=\"clearfix col-xs-12 ep-padding-left-none ep-vertical-align-center\"><span class=\"pull-left col-xs-10 {{item.captionClass}}\" title={{item.caption}} ng-bind=item.caption></span><!-- Favorite icon --> <i ng-if=\"(item.hideFavorite !== true)\" class=\"ep-menu-favorite fa fa-lg col-xs-2\" ng-click=\"toggleFavorite(item, $event)\" ng-class=\"{ 'fa-star-o': !item.favorite, 'fa-star text-warning': item.favorite}\"></i></span></div><div id=subMenuHeader class=\"list-group-item row\" ng-if=item.menuitems.length><!-- Menu item caption/text --><span class=\"clearfix col-xs-12 ep-padding-left-none ep-vertical-align-center\"><strong class=\"pull-left col-xs-10 {{item.captionClass}}\" title={{item.caption}} ng-bind=item.caption></strong><!-- Expand icon --> <i class=\"fa fa-lg col-xs-2\" ng-class=\"{ 'fa-caret-right': !item.isExpanded, 'fa-caret-down': item.isExpanded }\"></i></span></div></div><span class=\"row col-xs-10\" ng-click=\"navigate(item, false, $event)\" ng-if=\"item.description && (!item.menuitems.length || !item.isExpanded) && !hideDescription\"><sup class=text-info ng-bind=item.description></sup></span><!-- Sub-menu --><div class=list-group-submenu ng-class=\"{'collapsed': !item.isExpanded }\" id=mnu_children ng-if=\"item.isExpanded && item.menuitems.length\"><ep-accordion-menu-item ng-repeat=\"child in item.menuitems | orderBy:orderByMenu\" item=child hide-description=hideDescription commit-menu-state=commitMenuState navigate=navigate toggle-favorite=toggleFavorite></ep-accordion-menu-item></div></div>"
  );
